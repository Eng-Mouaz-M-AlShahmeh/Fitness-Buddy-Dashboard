    ...
    <div class="page-content">
        ...
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <span>{{__('dashboard.Edit Trainer Class')}}</span>
                </li>
            </ul>
        ...
        <!-- END PAGE BAR -->
        <!-- BEGIN PAGE TITLE-->
        <h1 class="page-title"> {{__('dashboard.Edit Trainer Class')}}
        </h1>
        <!-- END PAGE TITLE-->
        <!-- END PAGE HEADER-->
            ...
            <div class="col-md-12">
                ...
                <div class="portlet light portlet-fit portlet-form bordered">
                    ...
                    <div class="portlet-body">
                        <!-- BEGIN FORM-->
                        <form action="{{route('trainer.class.update',$trainerClass->id)}}" method="post" id="form_sample_1" class="form-horizontal">
                            @csrf
                            <div class="form-body">
                                    ...
                                    <div class="col-md-4">
                                        <select class="form-control" name="trainer_id">
                                            @foreach($trainers as $trainer)
                                                <option {{ $trainer->id == $trainerClass->trainer->id ? 'selected' : '' }} value="{{ $trainer->id }}">{{ $trainer->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-4">
                                        <select class="form-control" name="class_id">
                                            @foreach($classes as $class)
                                                <option {{ $class->id == $trainerClass->class->id ? 'selected' : '' }} value="{{ $class->id }}">{{ $class->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
