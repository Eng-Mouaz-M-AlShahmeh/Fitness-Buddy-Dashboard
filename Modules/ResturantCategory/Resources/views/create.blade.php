    ...
    <div class="page-content">
        ...
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    @if(App::isLocale('ar'))
                    <span>{{__('dashboard.Category')}} {{$resturantId->translate('ar')->name}}</span>
                    @else
                    <span>{{$resturantId->translate('en')->name}} {{__('dashboard.Category')}}</span>
                    @endif
                    <i class="fa fa-circle"></i>
                </li>
                <li>
                    <span>{{__('dashboard.Create Resturant Category')}}</span>
                </li>
            </ul>
        ...
        <!-- END PAGE BAR -->
        <!-- BEGIN PAGE TITLE-->
        <h1 class="page-title"> {{__('dashboard.Create Resturant Category Form')}}
        </h1>
        <!-- END PAGE TITLE-->
        <!-- END PAGE HEADER-->
            ...
            <div class="col-md-12">
                ...
                <div class="portlet light portlet-fit portlet-form bordered">
                    ...
                    <div class="portlet-body">
                        <!-- BEGIN FORM-->
                        <form action="{{route('resturant.categories.store',$resturantId->id)}}" method="POST" class="form-horizontal" enctype="multipart/form-data">
                            @csrf
...
{{--                                                <option value="{{$resturant->id}}">{{$resturant->name}} - {{$resturant->plan->name}}</option>--}}
{{--                                            @endforeach--}}
{{--                                        </select>--}}
{{--                                    </div>--}}
{{--                                </div>--}}
