...
@section('content')
    <div class="page-content">
        ...
        <!-- END PAGE BAR -->
        <!-- BEGIN PAGE TITLE-->
        <h1 class="page-title"> {{__('dashboard.Nutritionists Orders Datatable')}}
        </h1>
                                ...
                                @foreach ($orders as $key=>$order)
                                 @if(App::isLocale('ar'))
                                    <tr>
                                        ...
                                        <td> {{$order->nutritionist->translate('ar')->name}} </td>
                                        <td> {{$order->total}} </td>
                                        <td>
                                        ...
                                        <td>
{{--                                            <a href="{{ route('order.nutritionists.edit',[$order->id]) }}" class="btn btn-info btn-sm"><i class="fa fa-pencil"></i></a>--}}
                                            <a href="{{ route('order.nutritionists.show',[$order->id]) }}" class="btn btn-success btn-sm"><i class="fa fa-eye"></i></a>

                                            <form id="delete-form-{{ $order->id }}" action="{{ route('order.nutritionists.destroy',$order->id) }}" style="display: none;" method="POST">
                                                @csrf
                                            </form>
                                    ...
                                    <tr>
                                        ...
                                        <td> {{$order->id}} </td>
                                        <td> {{$order->user->name}} </td>
                                        <td> {{$order->nutritionist->translate('en')->name}} </td>
                                        <td> {{$order->total}} </td>
                                        <td>
                                            <div class="form-group">
                                                <div class="col-md-9">
...
@section('myjsfile')
    <script>
        function update_status(el){
            ...
            $.post('{{ route('order.nutritionists.status',isset($order) ? $order->id : "") }}', {_token:'{{ csrf_token() }}', id:el.value, accepted:accepted}, function(data){
                if(data == 1){
                    alert('{{__('dashboard.nutritionist accepted status changed')}}');
                }
                else{
