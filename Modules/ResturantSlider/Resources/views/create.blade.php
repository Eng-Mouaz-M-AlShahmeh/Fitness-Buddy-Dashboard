    ...
    <div class="page-content">
        ...
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    @if(App::isLocale('ar'))
                    <span>{{__('dashboard.Sliders')}} {{$resturant->translate('ar')->name}}</span>
                    @else
                    <span>{{$resturant->translate('en')->name}} {{__('dashboard.Sliders')}}</span>
                    @endif
                    <i class="fa fa-circle"></i>
                </li>

                <li>
                    <span> {{__('dashboard.Create Slider')}} </span>
                </li>
            </ul>
        ...
        <!-- END PAGE BAR -->
        <!-- BEGIN PAGE TITLE-->
        <h1 class="page-title">{{__('dashboard.Create Slider Form')}}
        </h1>
        <!-- END PAGE TITLE-->
        <!-- END PAGE HEADER-->
                ...
                <div class="portlet light portlet-fit portlet-form bordered">
                    <div class="portlet-title">
                        <div class="caption">
                            ...
                            <span class="caption-subject font-red sbold uppercase">{{__('dashboard.Slider Basic Validation')}} </span>
                        </div>

                    </div>
                    <div class="portlet-body">
                        <!-- BEGIN FORM-->
                        <form action="{{route('resturant.sliders.store',$resturant->id)}}" method="POST" class="form-horizontal" enctype="multipart/form-data">
                            @csrf
                            <input name="resturant_id" type="hidden" value="{{$resturant->id}}">
                            <div class="form-body">
